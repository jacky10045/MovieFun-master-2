...
class FavoriteTableViewController: UITableViewController {
    ...
    @objc func getMovieNoti(noti:Notification) {
        ...
        movies.insert(Movie, at: 0)
        if(Movie != nil){
            let alertController = UIAlertController(title: "完成!", message: "成功加入收藏", preferredStyle: .alert)
            ...
            let alertController = UIAlertController(title: "失敗!", message: "請再試一次", preferredStyle: .alert)
            let action = UIAlertAction(title: "確定", style: .default, handler: nil)
            alertController.addAction(action)
            present(alertController, animated: true, completion: nil)
        }
